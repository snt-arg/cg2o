...
time_to_show_fig = 1 # Time in seconds to show each figure
...
def main():
    if len(sys.argv) < 2:
        ...
        #sys.exit(1)
        filename = "260302_2241_AMPL_0_G2O_ISPD_gn_GN_13_3.bin"  # Default filename for testing
    else:    
       filename = sys.argv[1]
    ...
    cost_level_signal = np.array(safe_get(results, 'cost_level_signal', []))    
    u_input_signal_lang = np.array(safe_get(results, 'u_input_signal_lang', []))
    num_iterations_signal_lang = np.array(safe_get(results, 'num_iterations_signal_lang', []))
    cal_time_signal_lang = np.array(safe_get(results, 'cal_time_signal_lang', []))
    cost_level_signal_lang = np.array(safe_get(results, 'cost_level_signal_lang', []))
    ...
    if len(v_h_signal) > 0 or len(v_p_signal) > 0:
        ...
        ax1 = plt.subplot(number_rows, 2, 1)
        if len(v_p_signal) > 0:
        ...
        ax1.grid(True)
          ...
          # Subplot 2: Distance
        ax2 = plt.subplot(number_rows, 2, 2)
        if len(d_h_signal) > 0:
            ax2.plot(d_h_signal, 'g-')
        ax2.set_ylabel('Inter-distance (m)')
        ax2.grid(True)
        
        # Subplot 3: Iterations with mean/std
        ax3 = plt.subplot(number_rows, 2, 3)
        if len(num_iterations_signal) > 0:
            iter_mean = np.nanmean(num_iterations_signal)
            iter_std = np.nanstd(num_iterations_signal)
            ax3.plot(num_iterations_signal, 'y-', label='Iterations')
            ax3.plot([iter_mean]*len(num_iterations_signal), 'r-', label='Mean')
            ax3.plot([iter_mean + iter_std]*len(num_iterations_signal), 'r--')
            ax3.plot([iter_mean - iter_std]*len(num_iterations_signal), 'r--')
        ...
        ax4 = plt.subplot(number_rows, 2, 4)
        if len(cal_time_signal) > 0:
        ...
        #subplot 5: iterations_number_lang
        ax5 = plt.subplot(number_rows, 2, 5)
        if len(num_iterations_signal_lang) > 0:
            iter_mean_lang = np.nanmean(num_iterations_signal_lang)
            iter_std_lang = np.nanstd(num_iterations_signal_lang)
            ax5.plot(num_iterations_signal_lang, 'y-', label='Iterations Lang')
            ax5.plot([iter_mean_lang]*len(num_iterations_signal_lang), 'r-', label='Mean Lang')
            ax5.plot([iter_mean_lang + iter_std_lang]*len(num_iterations_signal_lang), 'r--')
            ax5.plot([iter_mean_lang - iter_std_lang]*len(num_iterations_signal_lang), 'r--')
        ax5.set_xlabel('Time steps')
        ax5.set_ylabel('Iterations Lang')
        ax5.legend()
        ax5.grid(True)  
        
        #subplot 6: cal_time_signal_lang
        ax6 = plt.subplot(number_rows, 2, 6)
        if len(cal_time_signal_lang) > 0:
            time_mean_lang = np.nanmean(cal_time_signal_lang)
            time_std_lang = np.nanstd(cal_time_signal_lang)
            ax6.plot(cal_time_signal_lang, 'c-', label='Comp Time Lang')
            ax6.plot([time_mean_lang]*len(cal_time_signal_lang), 'r-', label='Mean Lang')
            ax6.plot([time_mean_lang + time_std_lang]*len(cal_time_signal_lang), 'r--')
            ax6.plot([time_mean_lang - time_std_lang]*len(cal_time_signal_lang), 'r--')
            if print_results:
                print(f"Mean computation time (Lang): {time_mean_lang:.2f} ms")
        ax6.set_xlabel('Time steps')
        ax6.set_ylabel('Comp Time Lang (ms)')
        ax6.legend()
        ax6.grid(True)
        
        #subplot 7: control input signal
        ax7 = plt.subplot(number_rows, 2, 7)
        if len(u_input_signal) > 0:
            ax7.plot(u_input_signal, 'm-', label='Control Input')
        ax7.set_xlabel('Time steps')
        ax7.set_ylabel('Control Input (N)')
        ax7.legend()
        ax7.grid(True)      
        
        #subplot 8: control input signal lang
        ax8 = plt.subplot(number_rows, 2, 8)
        if len(u_input_signal_lang) > 0:
            ax8.plot(u_input_signal_lang, 'k-', label='Control Input Lang')
        ax8.set_xlabel('Time steps')
        ax8.set_ylabel('Control Input Lang (N)')
        ax8.legend()
        ax8.grid(True)  


        #subplot 9: cost_level_signal and cost_level_signal_lang
        ax9 = plt.subplot(number_rows, 2, 9)
        if len(cost_level_signal) > 0:
            ax9.plot(cost_level_signal, 'm-', label='Cost Level')
        if len(cost_level_signal_lang) > 0:
            ax9.plot(cost_level_signal_lang, 'k-', label='Cost Level Lang')
        ax9.set_xlabel('Time steps')    
        ax9.set_ylabel('Cost Level')
        ax9.legend()
        ax9.grid(True)      
        
        #
        diff_cost_signal = np.where(np.array(cost_level_signal) < np.array(cost_level_signal_lang), 1,
              np.where(np.array(cost_level_signal) > np.array(cost_level_signal_lang), 0, 0.5))
        
      
        
        #diff_cost_signal = np.array(cost_level_signal) - np.array(cost_level_signal_lang)

        #subplot 10: cost_level_signal < cost_level_signal_lang
        ax10 = plt.subplot(number_rows, 2, 10)
        if len(cost_level_signal) > 0 and len(cost_level_signal_lang) > 0:
            ax10.plot(diff_cost_signal, 'g-', label='Cost Level Diff')
        ax10.set_xlabel('Time steps')
        ax10.set_ylabel('Cost Level Diff')
        ax10.set_xlabel('Time steps')
        ax10.set_ylabel('Cost Level Diff')       
        ax10.legend()
        ax10.grid(True)
        ...
        if AlgIneqType == 'ISPD':   
            ...
            text_lines.extend([
                f"- Cont.: | gamma: {gamma:.2f} | limit_kappa_final: {limit_kappa_final:.1f} | slack_init_s: {init_slack_strategy:.0f} | lambda_0: {lambda_0:.1f} | s_0: {s_0:.1f} | error_pred_s: {error_pred_strategy:.0f} | error_pred_step_s: {ineq_prediction_step_strategy:.0f}"
        ])  
            text_lines.extend([
        ...
        try:
            if results['Alg_equality'] == 'al':
                ...
                text_lines.extend([
                     f"- Eq Params: max_num_inner_iterations: {settingEq5:.1f} | count of ones: {np.sum(diff_cost_signal == 1)} | of zeros: {np.sum(diff_cost_signal == 0)} | of 0.5: {np.sum(diff_cost_signal == 0.5)}"
                ])
        except KeyError:
            text_lines.extend([
                f"- Eq Params: max_num_inner_iterations: {settingEq5:.1f} | count of ones: {np.sum(diff_cost_signal == 1)} | of zeros: {np.sum(diff_cost_signal == 0)} | of 0.5: {np.sum(diff_cost_signal == 0.5)}"
        ...
        if len(num_iterations_signal) > 0 and len(num_iterations_signal_lang) > 0:
            text_lines.append(
                ...
                f"Iterations Lang: mean={np.nanmean(num_iterations_signal_lang):.1f} | min={np.nanmin(num_iterations_signal_lang)} | max={np.nanmax(num_iterations_signal_lang)} | σ={np.nanstd(num_iterations_signal_lang):.1f})"
            )

        # Add computation time if available
        if len(cal_time_signal) > 0 and len(cal_time_signal_lang) > 0:
            text_lines.append(
                f"- Comp Time: mean={np.nanmean(cal_time_signal):.1f}ms | min={np.nanmin(cal_time_signal):.1f} | max={np.nanmax(cal_time_signal):.1f} | σ={np.nanstd(cal_time_signal):.1f}ms | "
                f"Comp Time Lang: mean={np.nanmean(cal_time_signal_lang):.1f}ms | min={np.nanmin(cal_time_signal_lang):.1f} | max={np.nanmax(cal_time_signal_lang):.1f} | σ={np.nanstd(cal_time_signal_lang):.1f}ms"
            )

        # Add Time/Iteration statistics if available
        if len(cal_time_signal) > 0 and len(num_iterations_signal) > 0 and len(cal_time_signal_lang) > 0 and len(num_iterations_signal_lang) > 0:
            ct_times = np.array(cal_time_signal)
            iter_counts = np.array(num_iterations_signal)
            cal_time_per_iter = ct_times / iter_counts
            ct_mean = np.nanmean(cal_time_per_iter)
            ct_std = np.nanstd(cal_time_per_iter)
            
            ct_times_lang = np.array(cal_time_signal_lang)
            iter_counts_lang = np.array(num_iterations_signal_lang)
            cal_time_per_iter_lang = ct_times_lang / iter_counts_lang
            ct_mean_lang = np.nanmean(cal_time_per_iter_lang)
            ct_std_lang = np.nanstd(cal_time_per_iter_lang)
            
            text_lines.append(
                f"- Time/Iter: mean={ct_mean:.2f}ms | min={np.nanmin(cal_time_per_iter):.1f} | max={np.nanmax(cal_time_per_iter):.1f} | σ={ct_std:.2f}ms | "
                f"Time/Iter Lang: mean={ct_mean_lang:.2f}ms | min={np.nanmin(cal_time_per_iter_lang):.1f} | max={np.nanmax(cal_time_per_iter_lang):.1f} | σ={ct_std_lang:.2f}ms"
            )
