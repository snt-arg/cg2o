...
public:
  ...
  ACCMPCControlG2ONode() : Node("acc_mpc_control_g2o")
  {
    ...
    _mpcHorizon = getParameterByName(config_mpc_parameters_yaml_file_name, "N");

    // Declare parameters
     this->declare_parameter("numberOfIterations", 20); //change it back to 30

 
    this->declare_parameter<std::vector<double>>(
        "algIneqSettings", std::vector<double>{.5, 1500, 8}); // kappa_initial, kappa_final, update_factor
    // Get parameters
    _numberOfIterations = this->get_parameter("numberOfIterations")
                              .as_int();
    ...
    _controller = std::make_shared<ACCMPCControlG2O>(
        _numberOfIterations, algIneqSettings);
  ...
  ~ACCMPCControlG2ONode() {
    if (log_file_.is_open()) {
      ...
      log_file_.close();
      RCLCPP_INFO(this->get_logger(), "Log file closed: %s", log_filename_.c_str());
    }
  }
...
private:
  ...
  void initializeLogFile() {
    ...
    std::string install_dir = ament_index_cpp::get_package_share_directory("acc_control_mpc_g2o");
    std::filesystem::path pkg_path(install_dir);      
    std::filesystem::path workspace_path = pkg_path.parent_path().parent_path().parent_path().parent_path();    
    std::string log_dir = workspace_path.string() + "/data/logs/acc_control_mpc_g2o";
    RCLCPP_INFO(this->get_logger(), "Log directory will be: %s", log_dir.c_str());
    ...
    log_file_.open(log_filename_, std::ios::app);
     ...
     if (log_file_.is_open()) {
      ...
      log_file_ << "Started: " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << std::endl;
    ...
    void logActionToFile(const std::string& action_type,
                      double v_p, double a_p, double a_p_weighted, 
                      double v_h, double d_h, double force_prev, int horizon_length,
                      double result_force = 0.0, int flag_solved = 0, 
                      double cost_value = 0.0, int num_iterations = 0,
                      double solve_time_ms = 0.0, double desired_velocity = 0.0,
    ...
    if (action_type == "REQUEST") {
      ...
      log_file_ << "ros2 action send_goal /acc_control_mpc_g2o_action acc_interfaces/action/AccControlMPC \\" << std::endl;
      log_file_ << "\"{horizon_length: " << horizon_length 
                << ", v_p: " << std::fixed << std::setprecision(6) << v_p
                << ", v_h: " << v_h
                << ", a_p: " << a_p
                << ", a_p_weighted: " << a_p_weighted
                << ", d_h: " << d_h
      ...
      log_file_ << "result_force: " << std::fixed << std::setprecision(6) << result_force
                << ", flag_solved: " << flag_solved
                << ", cost_value: " << cost_value
                << ", num_iterations: " << num_iterations
                << ", solve_time_ms: " << solve_time_ms
                << ", desired_velocity: " << desired_velocity
                << ", desired_acceleration: " << desired_acceleration << std::endl;
      log_file_ << std::endl;
      log_file_ << "=======================================" << std::endl;
      log_file_ << std::endl;
    }
                    ...
                    rclcpp::Logger logger)
  {
    ...
    else
    {
      ...
      oss << " ***"
          << "road_grad_pred: " << prediction_data[1][0] << " "
              ...
              std::shared_ptr<const AccControlMPC::Goal> goal)
  {
    ...
    #ifndef NDEBUG
    logActionToFile("REQUEST", 
                    goal->v_p, goal->a_p, goal->a_p_weighted,
  ...
  void execute(const std::shared_ptr<GoalHandleAccControlMPC> goal_handle)
  {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Received parameters: v_p: %f, a_p: %f, a_p_weighted: %f, v_h: %f, d_h: %f, "
                "force_prev: %f, N: %d",
    ...
    RCLCPP_INFO(this->get_logger(),
                ...
                result->acc_input_force, result->num_iterations,
                result->solve_time);
  }
